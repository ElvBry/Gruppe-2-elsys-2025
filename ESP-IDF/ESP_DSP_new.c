...
#define SIGNAL_THRESHOLD_39kHz 200
#define NORMALIZED_THRESHOLD_39kHz 600000
#define BINARY_SEARCH_STEPS_39kHz 15
#define REF_TABLE_SIZE_39kHz 10

#define MAX_SIGNAL_DIFFERENCE_US 1000000
#define MAX_SIGNAL_DURATION_US 1500
...
// normalizes array to Q15 values before dividing by tablesize in order avoid problems in dotproduct function
static inline void normalize_signal_arr_Q15(int16_t *signal_arr, const uint16_t len, const uint8_t table_len) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        signal_arr[i] /= table_len; // avoid overflow in dotproduct calculation
    }
}
...
static inline bool signalPresent(const int16_t *src, const frequency_state_t *freq_state) { 
    ...
    uint64_t strength = calculate_signal_strength(
        freq_state->ref_cos, 
        freq_state->ref_sine, 
        &src[REDUCED_BUFFER_SIZE - freq_state->ref_table_len], 
...
static inline bool error_check_cycle(const int64_t maxCycleCount, const uint64_t startCycle, const uint8_t index) {
    ...
    if(startCycle + maxCycleCount < currentTime) {
        ESP_LOGE(TAG,"ERROR: cycleCount too high %u, cycle: %lld", index, currentTime-startCycle);
        return true;
    }
    return false;
}
...
static inline void update_frequency_state(int16_t *src, frequency_state_t *freq_state, const uint8_t index, const uint64_t end_time_us, const uint64_t startCycle) {
    ...
    if(!signalPresent(src, freq_state)) {
    ...
    if(estimated_start_index == 0xFF) return;
    ...
    */
    ...
    if(freq_state->arrival_times[index] == 0) { // First successful measurement of signal
        if(estimated_arrival_time_us > freq_state->last_ping_time_us) freq_state->last_ping_time_us = estimated_arrival_time_us;
        freq_state->arrival_times[index] = estimated_arrival_time_us;
        //return;
    }
...
static inline void process_buffer(uint16_t *src, uint64_t end_time_us, const uint64_t startCycle) {
    ...
    */
    ...
    for (int i = 0; i < NUM_FREQUENCIES; i++) {
        if(frequency_states[i].all_valid) {
            ESP_LOGI(TAG, "ALL FOUND: %lld", end_time_us);
            if((end_time_us > frequency_states[i].last_ping_time_us + MAX_SIGNAL_DURATION_US)) {
        ...
        if((frequency_states[i].last_ping_time_us > 0) && (end_time_us > frequency_states[i].last_ping_time_us + MAX_SIGNAL_DIFFERENCE_US)) {
            ...
            ESP_LOGI(TAG, "reset frequency max_difference");
            continue;
        }
    ...
    while(1) {
        ...
        if(xQueueReceive(dmaQueue, &event, portMAX_DELAY) == pdTRUE) {
            ...
            if (event.buf_id == BUFFER_PING) {
                ...
                process_buffer(buffer_ping, event.end_time_us, startCycleCount);
                ...
                */
                ...
                startCycleCount = get_ccount();
                process_buffer(buffer_pong, event.end_time_us, startCycleCount);
                endCycleCount = get_ccount();
