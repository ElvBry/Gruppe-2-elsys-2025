...
//#define USE_SIMPLE   // simple sum‐of‐abs over last 10 samples for testing
#define USE_DSPS     // optimized DSPS dot‐product
//#define USE_SQUARE_REF

#if defined(USE_SIMPLE) && defined(USE_DSPS)
  #error "Only one of USE_SIMPLE or USE_DSPS may be defined"
#endif
#if !defined(USE_SIMPLE) && !defined(USE_DSPS)
  #error "Define either USE_SIMPLE or USE_DSPS"
#endif
...
static inline void find_signal_present(int16_t *src, frequency_state_t *freq_state, const uint8_t index, const uint64_t end_time_us, const uint64_t startCycle) {
    ...
    */
    ...
    freq_state->valid_flags[index] = true; // uncomment to ignore second confirmation measurement
    for (int i = 0; i < NUM_RECEIVERS; i++) {
        if(!freq_state->valid_flags[i]) break;
        if(i == NUM_RECEIVERS -1) freq_state->all_valid = true;
    }
...
void app_main(void) {
    ...
    #ifdef USE_SQUARE_REF
    // Build a ±Q15‐max square: first half +max, second half -max
    const int16_t Q15MAX = 32767;
    for (int i = 0; i < REF_TABLE_SIZE_39kHz; i++) {
        ref_sine_table_Q15_39kHz[i] =
            (i < (REF_TABLE_SIZE_39kHz/2)) ?  Q15MAX : -Q15MAX;
        ref_cos_table_Q15_39kHz[i] = 0;
    }
    #endif
